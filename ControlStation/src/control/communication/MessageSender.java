...
/**
 * Runnable class to send messages. To be passed to an instance of
 * java.lang.Thread
 * 
 * @author Andy Barron
 */
public class MessageSender implements Runnable {
	 ...
	 * 
	 * @param control
	 *            the Controller associated with this Sender
	 * @param comm
	 *            the NXTComm using this sender
	 */
	 ...
	 * 
	 * @param message
	 *            the CommandMessage to send to the robot
	 * @throws IOException
	...
	/**
	 * Continually checks to see if the messages in the queue can be sent by
	 * calling transmit
	 */
	@Override
	public void run() {
		while (true) {
			if (!cmdQueue.isEmpty()) {
				try {
					...
					System.err.println("Failed to transmit message: "
							+ cmdQueue.peek().getFormattedMessage());
					e.printStackTrace();
					continue;
				}
				cmdQueue.remove();
